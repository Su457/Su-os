...
Su OS 是一个面向个人使用的响应式 Personal OS。v0.2.3 是可日常使用的本地优先工作台，并建立了最小自动化单元测试基线，用于集中管理任务、笔记、项目、学习数据、专注记录与 AI 助手界面。
...
- 半透明卡片、悬浮导航与柔和边缘高光
- 多层渐变光晕和细腻背景纹理
- 桌面端使用侧边栏，手机端使用紧凑页头与底部导航；“更多”菜单保留无需客户端 hydration 的原生交互降级
- 响应式布局，适配不同屏幕尺寸
...
- Learning 支持学习记录 CRUD、起止时间校验与自动时长计算，并提供桌面玻璃日期/时间面板和手机原生选择器
- Focus 完成后记录任务、项目、起止时间和专注分钟数
- Settings 支持 JSON 导入导出、恢复演示数据与清空本地数据
- AI 助手使用完整对话工作区，支持快捷问题、会话内消息、输入框自适应和本地演示回复，不连接真实模型
...
- 浏览器清理站点数据、无痕模式结束或更换设备都会导致本地数据不可用；v0.2.3 不提供跨设备同步
...
项目使用 Vitest 覆盖核心 Selectors、备份校验和 LocalStorage Repository 的单元测试。
...
v0.2.3 是可实际使用的本地优先版本，并将代码整理为 `Routes → Modules → Domain / Store → Data` 的结构。当前仓储保存的是整份浏览器本地快照；未来 Supabase 需要采用异步、表级 CRUD 和 Domain Mapper，不能把现有快照接口直接当作云仓储使用。
...
6. 修改完成后运行 `npm run lint`、`npm test` 和 `npm run build`。
