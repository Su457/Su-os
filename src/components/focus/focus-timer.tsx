...
export function FocusTimer() {
  const { data, addFocusSession, setFocusDraft } = useSuOsStore();
  const [seconds, setSeconds] = useState(SESSION_SECONDS);
  const [running, setRunning] = useState(false);
  const [startedAt, setStartedAt] = useState<string | null>(null);
  const [message, setMessage] = useState("");
  const draft = data.focusDraft;
  const task = data.tasks.find((item) => item.id === draft?.taskId);
  const project = data.projects.find((item) => item.id === (task?.projectId ?? draft?.projectId));
  const todayMinutes = data.focusSessions.filter((session) => toDateKey(new Date(session.completedAt)) === toDateKey()).reduce((sum, session) => sum + session.durationMinutes, 0);
  ...
  return (
    <article id="focus" className="card relative flex min-h-[350px] min-w-0 scroll-mt-24 flex-col overflow-hidden p-5 sm:p-6 lg:col-span-5">
      ...
      <div className="relative z-20 mt-5 grid min-w-0 grid-cols-1 gap-2 sm:grid-cols-2"><label className="min-w-0"><span className="field-label">关联任务</span><GlassSelect ariaLabel="关联任务" disabled={running} value={task?.id ?? ""} onChange={(value) => { const next = data.tasks.find((item) => item.id === value); setFocusDraft(next ? { taskId: next.id, projectId: next.projectId } : null); }} options={[{ value: "", label: "自由专注", meta: "不关联具体任务" }, ...data.tasks.filter((item) => !item.completed).map((item) => ({ value: item.id, label: item.title, meta: data.projects.find((projectItem) => projectItem.id === item.projectId)?.name ?? "无项目" }))]}/></label><label className="min-w-0"><span className="field-label">关联项目</span><GlassSelect ariaLabel="关联项目" disabled={running || Boolean(task)} value={project?.id ?? ""} onChange={(value) => setFocusDraft({ taskId: null, projectId: value || null })} options={[{ value: "", label: "无项目", meta: "自由专注" }, ...data.projects.filter((item) => item.status === "active").map((item) => ({ value: item.id, label: item.name, meta: item.goal || "进行中的项目" }))]}/></label></div>
