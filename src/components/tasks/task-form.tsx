...
import type { Project, Task, TaskInput, TaskPriority } from "@/lib/types";
  ...
  return (
    <form onSubmit={submit} className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <label><span className="field-label">日期</span><input type="date" value={dueDate} onChange={(event) => setDueDate(event.target.value)} className="field-input"/></label>
        <label><span className="field-label">时间</span><input type="time" value={time} onChange={(event) => setTime(event.target.value)} className="field-input"/></label>
      </div>
