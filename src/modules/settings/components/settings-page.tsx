...
export function SettingsPage() {
  ...
  if (!settings.hydrated) return <LoadingState/>;
  ...
  function exportJson() {
    const blob = new Blob([settings.exportData()], { type: "application/json" });
  ...
  async function importJson(event: ChangeEvent<HTMLInputElement>) {
    ...
    const result = settings.importData(await file.text());
  ...
  function resetDemo() {
    ...
    settings.resetDemoData(); showMessage("已恢复默认演示数据");
  }

  function clearData() {
    if (!window.confirm("这将清空全部任务、笔记、项目、学习和专注记录。是否继续？")) return;
    if (!window.confirm("最后确认：清空后无法撤销，确定继续吗？")) return;
    settings.clearAllData(); showMessage("全部本地数据已清空");
  }

  const counts = [
    ["任务", settings.counts.tasks], ["笔记", settings.counts.notes], ["项目", settings.counts.projects], ["学习记录", settings.counts.learningSessions], ["专注记录", settings.counts.focusSessions],
  ];
  ...
  return (
    <div className="mx-auto max-w-3xl">
      ...
      <p className="mt-5 text-center text-[10px] text-[#53535e]">Su OS · Local-first v0.2.1</p>
    </div>
  );
}
